...
public interface TweetDao {
    ...
    @Query(
            "SELECT Tweet.body AS tweet_body, Tweet.createdAt AS tweet_createdAt, Tweet.id AS tweet_id, User.* " +
                    "FROM Tweet INNER JOIN User ON Tweet.userId == User.id ORDER BY Tweet.createdAt DESC LIMIT 9,20")
